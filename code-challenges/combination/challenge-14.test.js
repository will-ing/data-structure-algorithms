...
const toTitleCase = (arr) => {
  return arr.map(v => v.replace(/^\w/, c => c.toUpperCase()));
};
...
let biggerThanLuke = (arr) => {
  var luke = arr.filter(v => v.name === 'Luke Skywalker');
  return arr.reduce((a, character) => {
    if(character.mass > parseInt(luke[0].mass)){
      a.push(character.name);
    }
    return a;
  }, []).join(' - ');
};
