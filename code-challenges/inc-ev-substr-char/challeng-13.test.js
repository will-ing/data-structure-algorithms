...
const onlyOddChars = (str) => {
  return str.split('').reduce((a, v, i) => i % 2 !== 0 ? a += v : a, '');
};
...
const allHappy = (arr) => {
  let check = arr.filter(v => v.includes(':)') ? true : false);
  return check.length === arr.length ? true : false;
};
...
const findAnything = (arr, target) => {
  return arr.filter(v => v.includes(target) ? v : null);
};
...
const findEvery = (arr, target) => {
  let check = arr.filter(v => v.includes(target) ? true : false);
  return check.length === arr.length ? true : false;
};
...
const unenrollBrook = (arr) => {
  return arr.map(v => {
    return v.filter(s => s.includes('Brook') ? false : s);
  });
};
