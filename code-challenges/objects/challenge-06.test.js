...
const getHouses = (arr) => {
  let houses = [];
  Object.keys(arr).forEach(house => {
    houses.push(arr[house]['house']);
  });
  return houses;
};
...
const hasChildrenValues = (arr, character) => {
  ...
  Object.values(arr).forEach(characters => {
    characters.name === character && characters.children !== [] ? result = true : false;
  });
  return result;
};
...
const hasChildrenEntries = (arr, character) => {
  let result = false;
  Object.entries(arr).forEach(house => {
    house[1].name === character && house[1].children !== [] ? result = true : false;
  });
  return result;
};
